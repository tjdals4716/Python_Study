...
# 입력 이미지 전처리: 히스토그램 평활화 적용
img_left_eq = cv2.equalizeHist(img_left)
img_right_eq = cv2.equalizeHist(img_right)
...
# 시차 맵 계산
disparity = stereo.compute(img_left_eq, img_right_eq).astype(np.float32) / 16.0

# 중간값 필터 적용
disparity_filtered = cv2.medianBlur(disparity, 5)

# 시차 맵을 보기 좋게 조정 (시각화를 위해)
disparity_normalized = cv2.normalize(disparity_filtered, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX)
